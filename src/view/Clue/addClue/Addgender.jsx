...
class AddgradeCpts extends React.Component {
    constructor(props) {
        ...
        const { value, label } = props.linkManGrade
    ...
    handleChange  (i)  {
        ...
        const gender = this.props.linkManGrade
    ...
    render() {
        ...
        return (
            <div className="addClue addClue-grade">
                <Flex.Item className="page-title" >联系人性别</Flex.Item>
...
*/}                <LinkNextStep to={`${linkUrl}`}
                                 ToastInfo={`请录入联系人性别`}
                                 linkEnabled={linkEnabled} onClick={this.handleLinkClick} >{btnText}</LinkNextStep>
            </div>
        )
    }
}
...
const mapStateToProps = (state) => ({
    linkManGrade: state.linkManGrade
})
