...
class HandleSpecial extends React.Component {
	...
	getCCinfo() {
		...
		let { specialId, applyType, applyContent, length, } = this.state;
		...
		if(this.props.specialId != undefined) 
		...
		Http.ajax(`${api}/changgui/get-special`,params).then(res => {
			if(res.code == 0) {
				...
				let auditStaffs = data.auditStaffs;
	...
	back() {
		if(this.props.specialId != undefined)
			...
			this.props.history.push(`/ContractDetail/${this.props.history.location.hash.substring(1,this.props.history.location.hash.length)}`);
	}
	...
	render() {
		...
		return(
			<div className="special-detail handle-special special-css">
				...
				<a className="l-btn" href="javascript:void(0)" onClick={ () => this.back() }>返回</a>	
			</div>
		)
	}
}
